...
function SubmitButton() {
    ...
    if (!user) {
      ...
      if (form) {
        ...
        formData.forEach((value, key) => {
          if (key !== 'idToken') { // Don't store the empty token
  ...
  if (pending) {
    return <Button type="submit" className="w-full" disabled={true}>Đang gửi...</Button>;
  }
          ...
          setTimeout(() => {
            ...
            try {
              ...
              if (formRef.current) {
                ...
                (formRef.current.elements.namedItem('currentJob') as HTMLInputElement).value = savedData.currentJob || '';
                (formRef.current.elements.namedItem('question') as HTMLTextAreaElement).value = savedData.question || '';
              }
          ...
          <CardContent>
            <form ref={formRef} action={formAction} className="space-y-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="name">Họ và Tên<RequiredIndicator /></Label>
                <Input id="name" name="name" placeholder="Nguyễn Văn A" value={formData.name} onChange={handleInputChange} required />
                {state.errors?.name && <p className="text-sm text-destructive">{state.errors.name[0]}</p>}
              </div>
              <div className="space-y-2">
                <Label htmlFor="email">Email<RequiredIndicator /></Label>
                <Input id="email" name="email" placeholder="nguyenvana@email.com" value={formData.email} onChange={handleInputChange} required type="email" readOnly={!!user?.email} className={!!user?.email ? 'bg-muted' : ''} />
                {state.errors?.email && <p className="text-sm text-destructive">{state.errors.email[0]}</p>}
              </div>
              <div className="space-y-2">
                <Label htmlFor="phone">Số điện thoại<RequiredIndicator /></Label>
                <Input id="phone" name="phone" type="tel" placeholder="09xxxxxxxx" value={formData.phone} onChange={handleInputChange} required />
                {state.errors?.phone && <p className="text-sm text-destructive">{state.errors.phone[0]}</p>}
              </div>
              <div className="space-y-2">
                <Label htmlFor="currentJob">Công việc hiện tại</Label>
                <Input id="currentJob" name="currentJob" placeholder="Quant Trader tại ..." value={formData.currentJob} onChange={handleInputChange} />
              ...
              <div className="space-y-2">
                ...
                <Textarea id="question" name="question" placeholder="Tôi muốn hỏi về..." value={formData.question} onChange={handleInputChange} />
              ...
              {state.errors?.idToken && <p className="text-sm text-center text-destructive">{state.errors.idToken[0]}</p>}
            </form>
          </CardContent>
        </Card>
      </div>
